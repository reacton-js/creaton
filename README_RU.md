...
Все специальные свойства и методы, начинаются с символа доллара «$», за которым следует название метода или свойства.
...
*Этот метод не обновляет содержимое закрытых компонентов, вне статических методов класса компонента. Для всех остальных типов компонентов, он возвращает количество миллисекунд, за которое содержимое компонента было обновлено.*
...
*Этот метод не выбирает содержимое закрытых компонентов, вне статических методов класса компонента.*
...
static connected() {
  ...
  // перебрать коллецию элементов в цикле
...
Прокси объекта состояния изначально ищет свойство в самом объекте состояния, это значит, что для получения одноимённого свойства из элемента компонента, необходимо использовать специальное свойтсво *$host*, как показано ниже:
