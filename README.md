...
Метод **render()** возвращает представление компонента в виде строки. На основе этого содержимого, будет создано *HTML*-содержимое компонента. Вместо функции возвращающей строковое представление, можно передать свойству **render** элемент [*Template*](https://learn.javascript.ru/template-element), как показано ниже:
...
При использовании элемента *Template*, обратные кавычки внутри содержимого необходимо экранировать, как в настоящих шаблонных строках:
...
Компонентов может быть множество и они могут содержать различные асинхронные методы. Импортированный модуль Creaton возвращает [промис](https://learn.javascript.ru/promise-basics), который разрешается после загрузки всех компонентов на страницу и выполнения в них всех асинхронных методов, например:
...
Свойство **extends** позволяет [модифицировать](https://learn.javascript.ru/custom-elements#modifitsirovannye-vstroennye-elementy) компонент таким образом, чтобы при его монтировании можно было бы использовать стандартный HTML-элемент. Название тега данного элемента указывается в нижнем регистре, например:

```js
extends: 'header'
```

При этом, такой HTML-элемент обязательно должен содержать атрибут ***is*** со значением, соответствующем названию компонента:
...
Свойство **mode** определяет уровень инкапсуляции [Теневого DOM](https://learn.javascript.ru/shadow-dom) компонента, который используется для изоляции его [стилей](https://learn.javascript.ru/shadow-dom-style) от всего остального Документа и других Веб-компонентов, например:

```js
mode: 'closed'
```

Данное свойство может иметь одно из двух значений:

1. "open" – корень теневого дерева («shadow root») доступен как elem.shadowRoot
2. "closed" – elem.shadowRoot всегда возвращает null

По умолчанию, компоненту не добавляется Теневой DOM. Кроме этого, данное свойство можно использовать лишь с [некоторыми](https://developer.mozilla.org/ru/docs/Web/API/Element/attachShadow), модифицированными при помощи свойства **extends** компонентами.

Например, компонент может быть модифицирован во встроенный HTML-элемент *Header*, который допускает использование Теневого DOM, как показано ниже:
...
Однако, многие стандартные HTML-элементы, такие, например, как *Button* или *Form*, не позволяют добавлять к себе Теневой DOM. Попытка сделать это приведёт к ошибке:
...
Чтобы компонент *Hello* можно было модифицировать в кнопку, свойство **mode** необходимо удалить:

```html
<body>
  <!-- монтировать компонент Hello в элемент Button -->
  <button is="my-hello"></button>
  ...
  <script type="module">
    ...
    const Hello = {
      name: 'my-hello',
      // модифицировать компонент для его монтирования в элемент Button
      extends: 'button',
      data() {
        return {
          message: 'Creaton'
...
Кроме этого, использование [слотов](https://learn.javascript.ru/slots-composition) возможно лишь для компонентов с Теневым DOM, например:

```html
<body>
  <!-- монтировать компонент Hello в элемент Header -->
  <header is="my-hello">
    <p slot="text">Содержимое именованного слота text</p>
...
Внесите в файл *index.html* следующие изменения:
...
Метод **$render()** возвращает промис, результатом которого является количество миллисекунд затраченное на обновление DOM компонента:

```js
changed() {
  /* обновить DOM компонента и вывести количество миллисекунд,
    за которое произошло его обновление */
  this.$render().then(milliseconds => console.log(milliseconds))
}
```
...
После изменения значения наблюдаемого атрибута, экран браузера будет сразу же обновлён, поскольку в методе **changed()** присходит вызов специального метода **$render()**, который полностью переписывает содержимое DOM компонента:
...
Кроме этого, если бы мы использовали не отслеживаемые атрибуты, а обычные пользовательские свойства объекта данных компонента, не задействуя метод **changed()**, например:
...
То после изменения значений в этих пользовательских свойствах, необходимо было бы вызвать метод **$render()** вручную, как показано ниже:

```
hello.$data.message = 'Мир'
hello.$data.color = 'green'
hello.$render()
```
...
Свойство **$data** ссылается на возвращаемый методом **data()** объект данных компонента:
...
Свойство **$root** ссылается на Теневой DOM компонента, если свойству **mode** было присвоено значение "open" или "closed". В этом случае, свойство **\$root** будет равно свойству **shadowRoot** элемента компонента:

```
hello.$root === hello.shadowRoot
```

Если же свойству **mode** не задавалось значение, то специальное свойство **$root** ссылается на сам элемент компонента:

```
hello.$root === hello
```

Кроме специальных, в Creaton имеется и два вспомогательных метода, которые служат для быстрого доступа к содержимому DOM компонента. Доступ к содержимому DOM компонента можно получить после его определения на странице.
...
Во всех трёх показанных случаях, обработчик события выполнится всего один раз. Это объясняется тем, что после выполнения метода **$render()**, старое содержимое DOM компонента удаляется и создаётся новое содержимое, в котором, новой кнопке уже не назначен никакой обработчик.
...
В данном примере, внутри метода **connected()** компонента *Hello*, с помощью метода **\$()** выбирается из его DOM-содержимого элемент компонента *ArrComponent*. После получения элемента компонента, мы имеем доступ ко всем его специальным методам и свойствам, в том числе, к свойству **\$data** и к методу **$render()**, как показано ниже:
...
После нажатия на кнопку, происходит обращение массива и обновление DOM компонента *ArrComponent*, не затрагивая содержимого самого компонента *Hello*, ввиду чего, обработчик кнопки будет работать множество раз, поскольку кнопка больше не удаляется из DOM компонента *Hello*.
...
Внутри этого обработчика происходит обращение массива **arr** и обновление DOM компонента *ArrComponent*.
